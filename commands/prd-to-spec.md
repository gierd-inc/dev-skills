...
1. **Resolve the slug.**
   - If `$ARGUMENTS` is empty: read `.dev-skills/CURRENT` to get the active slug. If no CURRENT, error and tell the user to pass a PRD source.
   - If `$ARGUMENTS` is a slug that already exists under `.dev-skills/`: treat as a re-run on that slug.
...
2. **Scaffold (new slug only):**
   `bash scripts/new-feature.sh <slug> <type> <ref> <spec_file>`. This creates `.dev-skills/<slug>/` and writes `.dev-skills/CURRENT`.
...
5. **Dispatch the architect agent** (Task tool, `subagent_type=architect`) with prompt:
   `FEATURE_SLUG=<slug>. Read .dev-skills/<slug>/spec-source.md (and existing spec.md if any). Grill the user and produce/refine spec.md.`

6. Record usage: `bash -c "source scripts/usage.sh && usage_record '<slug>' 'architect' '' 'opus' '$ARCH_START' '.dev-skills/<slug>/spec-source.md' '.dev-skills/<slug>/spec.md'"`
