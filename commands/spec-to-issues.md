...
2. Verify `.dev-skills/<slug>/spec.md` exists. If not, tell the user to run `/prd-to-spec` first and stop.
...
5. Record usage: `bash -c "source scripts/usage.sh && usage_record '<slug>' 'designer' '' 'sonnet' '$DESIGN_START' '.dev-skills/<slug>/spec.md' '.dev-skills/<slug>/design/proposal.md'"`
