...
2. Verify prerequisites:
   - `.dev-skills/<slug>/spec.md` exists.
   - `.dev-skills/<slug>/issues/` is non-empty.
...
- **Append a one-line entry to `.dev-skills/<slug>/team-lead/log.md`** for every meaningful action (timestamp + action). For non-happy-path decisions, also write `team-lead/decisions-NNN.md`.
...
- engineer dispatches: model=`sonnet`, input files=`spec.md,.dev-skills/<slug>/issues/<file>`, output=work-log.
...
3. `mkdir -p .dev-skills/<slug>/engineer/$ID`.
4. Record start time: `ENG_START="$(date +%s)"`.
5. Dispatch `engineer` via Task (`subagent_type=engineer`):
   ```
   FEATURE_SLUG=<slug>
   ISSUE_ID=$ID
   ISSUE_FILE=.dev-skills/<slug>/issues/<file from state.issues[].file>
   WORKTREE_PATH=<abs path>
   OUTPUT_FILE=.dev-skills/<slug>/engineer/$ID/work-log-NNN.md
   SKILLS_USED_FILE=.dev-skills/<slug>/engineer/$ID/skills-used.md
   PRIOR_REVIEW_FILE=<.dev-skills/<slug>/review/$ID/summary-(NNN-1).md if it exists, else empty>
   ```
6. Re-read `OUTPUT_FILE` and `SKILLS_USED_FILE` after the agent returns.
7. Record usage: `bash -c "source scripts/usage.sh && usage_record '<slug>' 'engineer' '$ID' 'sonnet' '$ENG_START' '.dev-skills/<slug>/spec.md,.dev-skills/<slug>/issues/<file>' '.dev-skills/<slug>/engineer/$ID/work-log-NNN.md'"`
...
4. For each skill: `mkdir -p .dev-skills/<slug>/review/$ID/<skill>` and compute that skill's per-skill round number `MMM` (1 + count of existing `review-*.md` in that dir).
...
6. **Dispatch all reviewers in parallel** — issue every Task call in a single message, one `code-reviewer` per skill:
   ...
   SKILL=<skill>
   WORKTREE_PATH=<abs path>
   OUTPUT_FILE=.dev-skills/<slug>/review/$ID/<skill>/review-MMM.md
   ```
7. After all reviewers return, record usage for each reviewer:
   `bash -c "source scripts/usage.sh && usage_record '<slug>' 'code-reviewer' '$ID' 'haiku' '$REV_START' '.dev-skills/<slug>/engineer/$ID/work-log-NNN.md' '.dev-skills/<slug>/review/$ID/<skill>/review-MMM.md'"`
   (run once per skill, or approximate with a single call covering the whole fan-out using the summary as output_file)
8. Re-read every output file and write `.dev-skills/<slug>/review/$ID/summary-NNN.md`:
   ```markdown
...
4. Dispatch `validator` via Task (`subagent_type=validator`):
   ...
   MODE=impl
   WORKTREE_PATH=<abs path>
   OUTPUT_FILE=.dev-skills/<slug>/validation/final-NNN.md
   ```
5. Record usage: `bash -c "source scripts/usage.sh && usage_record '<slug>' 'validator' '' 'opus' '$VAL_START' '.dev-skills/<slug>/spec.md' '.dev-skills/<slug>/validation/final-NNN.md'"`
...
Categorize findings into impl-level / design-level / spec-level. Write `.dev-skills/<slug>/team-lead/triage-NNN.md` with the categorization.

- **All findings impl-level**: open new fixup issues. For each finding, write `.dev-skills/<slug>/issues/issue-fixup-<seq>.md` and `state_add_issue <slug> <id> issues/<file> "<deps>" "<skills>"`. Set `state_set <slug> .phase '"building"'`. Loop back to **Per-issue loop**.
...
2. Create a **draft** PR:
   ...
   - Body: structured summary linking `.dev-skills/<slug>/spec.md` (relative to repo root) and listing each shipped issue.
